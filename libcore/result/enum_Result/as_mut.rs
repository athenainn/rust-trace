...
mod tests {
    ...
    #[test]
    fn as_mut_test1() {
	let mut x: Result<T, E> = Ok::<T, E>(2);
	let result: Result<&mut T, &mut E> = x.as_mut();
	...
	assert_eq!(result, Ok::<&mut T, &mut E>(&mut 42));
    }

    #[test]
    fn as_mut_test2() {
	let mut x: Result<T, E> = Err::<T, E>(13);
	let result: Result<&mut T, &mut E> = x.as_mut();
	...
	assert_eq!(result, Err::<&mut T, &mut E>(&mut 0));
    }
}
