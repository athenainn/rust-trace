...
mod tests {
	...
	atomicisize.store(val, Acquire); // panicked at 'there is no such thing as an acquire store'
    }
	...
	atomicisize.store(val, AcqRel); // panicked at 'there is no such thing as an acquire/release store'
    }
}
