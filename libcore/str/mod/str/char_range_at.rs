...
mod tests {
    ...
    #[test]
    fn char_range_at_test1() {
	let x: &str = "精誠";

	assert_eq!(x.len(), 6);

	let result: CharRange = x.char_range_at(0);
	assert_eq!(result.ch, '精');
	assert_eq!(result.next, 3);

	let result: CharRange = x.char_range_at(3);
	assert_eq!(result.ch, '誠');
	assert_eq!(result.next, 6);
    }

    #[test]
    #[should_panic]
    fn char_range_at_test2() {
	let x: &str = "銹";

	assert_eq!(x.len(), 3);

	let result: CharRange = x.char_range_at(0);
	assert_eq!(result.ch, '銹');
	assert_eq!(result.next, 3);

	let _: CharRange = x.char_range_at(1); // panicked at 'assertion failed: w != 0'
    }

    #[test]
    #[should_panic]
    fn char_range_at_test3() {
	let x: &str = "銹";
	let _: CharRange = x.char_range_at(2); // panicked at 'assertion failed: w != 0'
    }

    #[test]
    #[should_panic]
    fn char_range_at_test4() {
	let x: &str = "銹";
	let _: CharRange = x.char_range_at(3); // panicked at 'index out of bounds: the len is 3 but the index is 3'
    }
}
