...
mod tests {
    ...
    fn init_mut_test1() {
	let slice: &mut [T] = &mut [];
	let _: &mut [T] = slice.init_mut(); // panicked at 'arithmetic operation overflowed'
    }
	...
	{
	    let init_mut: &mut [T] = slice.init_mut();

	    for i in 0..init_mut.len() {
		init_mut[i] += 10;
	    }
	}

	assert_eq!(slice, &mut [21, 22, 22, 23, 24, 25, 16]);
    }
}
