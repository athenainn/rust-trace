...
mod tests {
	...
	($init:expr, $val:expr, $xorder:ident, $result:expr) => ({
		...
		let result: isize = clone.fetch_xor(val, $xorder);
		assert_eq!(result, $init);
	    });
    ...
    #[test]
    fn fetch_xor_test1() {
	fetch_xor_test!( 0, 0, Relaxed, 0 );
	fetch_xor_test!( 0, 0, Release, 0 );
	fetch_xor_test!( 0, 0, Acquire, 0 );
	fetch_xor_test!( 0, 0, AcqRel, 0 );
	fetch_xor_test!( 0, 0, SeqCst, 0 );
    }
