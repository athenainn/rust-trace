...
mod tests {
	...
	($init:expr, $current:expr, $new:expr, $order:ident, $result:expr) => ({
	    let atomicisize: AtomicIsize = AtomicIsize::new($init);

	    let result: isize = atomicisize.load(Relaxed);
	    assert_eq!(result, $init);

	    let data: Arc<AtomicIsize> = Arc::<AtomicIsize>::new(atomicisize);
	    let clone: Arc<AtomicIsize> = data.clone();

	    let result: isize = clone.load(Relaxed);
	    assert_eq!(result, $init);

	    thread::spawn(move || {
		let current: isize = $current;
		let new: isize = $new;

		let result: isize = clone.load(Relaxed);
		assert_eq!(result, $init);

		let result: isize = clone.compare_and_swap(current, new, $order);
		assert_eq!(result, $init);
	    });

	    thread::sleep_ms(10);

	    let result: isize = data.load(Relaxed);
	...
	compare_and_swap_test!( 0, 0, 68, Relaxed, 68 );
	compare_and_swap_test!( 0, 0, 68, Release, 68 );
	compare_and_swap_test!( 0, 0, 68, Acquire, 68 );
	compare_and_swap_test!( 0, 0, 68, AcqRel, 68 );
	compare_and_swap_test!( 0, 0, 68, SeqCst, 68 );
    }

    #[test]
    fn compare_and_swap_test2() {
	compare_and_swap_test!( 0, 68, 68, Relaxed, 0 );
	compare_and_swap_test!( 0, 68, 68, Release, 0 );
	compare_and_swap_test!( 0, 68, 68, Acquire, 0 );
	compare_and_swap_test!( 0, 68, 68, AcqRel, 0 );
	compare_and_swap_test!( 0, 68, 68, SeqCst, 0 );
    }

    #[test]
    fn compare_and_swap_test3() {
	compare_and_swap_test!( 68, 0, 68, Relaxed, 68 );
	compare_and_swap_test!( 68, 0, 68, Release, 68 );
	compare_and_swap_test!( 68, 0, 68, Acquire, 68 );
	compare_and_swap_test!( 68, 0, 68, AcqRel, 68 );
	compare_and_swap_test!( 68, 0, 68, SeqCst, 68 );
    }

    #[test]
    fn compare_and_swap_test4() {
	compare_and_swap_test!( 68, 68, 0, Relaxed, 0 );
	compare_and_swap_test!( 68, 68, 0, Release, 0 );
	compare_and_swap_test!( 68, 68, 0, Acquire, 0 );
	compare_and_swap_test!( 68, 68, 0, AcqRel, 0 );
	compare_and_swap_test!( 68, 68, 0, SeqCst, 0 );
    }
}
