...
mod tests {
	...
	($value:expr, $rhs:expr, $result:expr) => ({
	    let x: u8 = $value;
	    let rhs: u8  = $rhs;
	    let result: u8 = x.wrapping_div(rhs);
	...
	let x: u8 = 68;
	let rhs: u8  = 0;
	let _: u8 = x.wrapping_div(rhs); // panicked at 'attempted to divide by zero'
    }

    #[test]
    fn wrapping_div_test2() {
	wrapping_div_test!( 0xff, 0x01, 0xff );
	wrapping_div_test!( 0xff, 0x02, 0x7f );
	wrapping_div_test!( 0xff, 0x03, 0x55 );
    }
}
