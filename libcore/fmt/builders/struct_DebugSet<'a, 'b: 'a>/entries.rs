...
mod tests {
	...
	fn fmt(&self, fmt: &mut Formatter) -> Result {
	    let iter: Iter<T> = self.0.iter();
	    let mut debugset: DebugSet = fmt.debug_set();

	    type D<'a> = &'a T;
	    type I<'a> = Iter<'a, T>; // I: IntoIterator<Item=D>
	    let debugset_refmut: &mut DebugSet = debugset.entries::<D, I>(iter);
	    let result: Result = debugset_refmut.finish();
