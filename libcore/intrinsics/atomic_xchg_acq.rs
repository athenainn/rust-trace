...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xchg_acq;
    ...
    macro_rules! atomic_xchg_acq_test {
	($init:expr, $value:expr) => ({
		...
		let src: T = $value;
		let result: T = unsafe { atomic_xchg_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xchg_acq_test1() {
	atomic_xchg_acq_test!(  68, 500 );
    }
}
