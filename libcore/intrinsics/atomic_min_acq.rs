...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_min_acq;
    ...
    macro_rules! atomic_min_acq_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_min_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_min_acq_test1() {
	atomic_min_acq_test!( !0, 0, !0 );
    }
}
