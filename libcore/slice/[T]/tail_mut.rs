...
mod tests {
    ...
    fn tail_mut_test1() {
	let slice: &mut [T] = &mut [];
	let _: &mut [T] = slice.tail_mut(); // panicked at 'assertion failed: index.start <= index.end'
    }
	...
	{
	    let tail_mut: &mut [T] = slice.tail_mut();

	    for i in 0..tail_mut.len() {
		tail_mut[i] += 10;
	    }

	    assert_eq!(tail_mut, &mut [22, 22, 23, 24, 25, 26]);
	}

	assert_eq!(slice, &mut [11, 22, 22, 23, 24, 25, 26]);
    }
}
