...
mod tests {
    ...
    #[test]
    fn add_test1() {
	let v1: Digit32 = 0x12345678;
	let mut big: Big32x36 = Big32x36::from_small(v1);

	let v2: Digit32 = 0xfedcba98;
	let other: Big32x36 = Big32x36::from_small(v2);

	let add: &Big32x36 = big.add(&other);

	let result: String = format!("{:?}", add);
	assert_eq!(result, "0x1_11111110".to_string());
    }
	...
	let mut big: &mut Big32x36 = big.mul_digits(other);

	let other: Big32x36 = Big32x36::from_small(v);

	let _: &Big32x36 = big.add(&other); // panicked at 'index out of bounds: the len is 36 but the index is 36'
    }
}
