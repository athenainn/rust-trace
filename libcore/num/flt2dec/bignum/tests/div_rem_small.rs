...
mod tests {
	...
	let v: u8 = 0x01;
	let mut big: Big8x3 = Big8x3::from_small(v);
	let other: &[u8] = &[ 0x34, 0x12 ];
	let mul_digits: &mut Big8x3 = big.mul_digits(other);

	let other: u8 = 16;
	let (quotient, remainder): (&mut Big8x3, u8) = mul_digits.div_rem_small(other);

	let result: String = format!("{:?}", quotient);
	assert_eq!(result, "0x1_23".to_string());
	...
	let other: u8 = 0;
	let _: (&mut Big8x3, u8) = mul_digits.div_rem_small(other); // panicked at 'assertion failed: other > 0'
    }
}
