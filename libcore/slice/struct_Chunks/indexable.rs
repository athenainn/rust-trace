...
mod tests {
	...
	let slice: &[T] = &[11, 12, 12, 13, 14, 15, 16];
	let size: usize = 1;
	let chunks: Chunks<T> = slice.chunks(size);
	let indexable: usize = chunks.indexable();

	assert_eq!(indexable, 7);
    }
