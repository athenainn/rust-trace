...
mod tests {
	...
	assert_eq!(result.next(), Some::<&str>("葡萄倒吐葡萄"));
	assert_eq!(result.next(), Some::<&str>("不"));
	assert_eq!(result.next(), Some::<&str>("葡萄不吐葡萄"));
	assert_eq!(result.next(), Some::<&str>(""));
	assert_eq!(result.next(), None::<&str>);
    }

    #[test]
    fn rsplit_terminator_test4() {
