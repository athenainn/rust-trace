...
mod tests {
    ...
    #[test]
    fn trim_left_matches_test1() {
	type P<'a> = &'a str; // P: Pattern<'a>

	let x: &str = "-=<勤能補拙>=-";
	let pat: P = "-=<";
	let result: &str = x.trim_left_matches::<P>(pat);

	assert_eq!(result, "勤能補拙>=-");
    }

    #[test]
    fn trim_left_matches_test2() {
