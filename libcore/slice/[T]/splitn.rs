...
mod tests {
	...
	let n: usize = 0;
	let pred: P = P;
	let mut splitn: SplitN<T, P> = slice.splitn::<P>(n, pred);

	assert_eq!(splitn.next(), None::<&[T]>);
    }
	...
	assert_eq!(splitn.next(), Some::<&[T]>(slice));
	assert_eq!(splitn.next(), None::<&[T]>);
    }
	...
	assert_eq!(splitn.next(), Some::<&[T]>(&[11]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[12, 13, 14, 15, 16]));
	assert_eq!(splitn.next(), None::<&[T]>);
    }
	...
	assert_eq!(splitn.next(), Some::<&[T]>(&[11]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[13, 14, 15, 16]));
	assert_eq!(splitn.next(), None::<&[T]>);
    }
	...
	assert_eq!(splitn.next(), Some::<&[T]>(&[11]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[13, 14]));
	assert_eq!(splitn.next(), Some::<&[T]>(&[16]));
	assert_eq!(splitn.next(), None::<&[T]>);
    }
