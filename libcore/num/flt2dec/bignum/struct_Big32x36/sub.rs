...
mod tests {
	...
	let v2: Digit32 = 0x00005678;
	let other: Big32x36 = Big32x36::from_small(v2);

	let sub: &Big32x36 = big.sub(&other);

	let result: String = format!("{:?}", sub);
	assert_eq!(result, "0x12340000".to_string());
    }
	...
	let _: &Big32x36 = big.sub(&other); // panicked at 'assertion failed: noborrow'
    }
}
