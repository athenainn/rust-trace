...
mod tests {
...

	{
	    ...
	    for i in 0..len {
		let x: Option<&mut [T]> = chunks_mut.next();
		match x {
		    Some(v) => {
			for j in 0..v.len() {
			    v[j] = i as T;
	...
	assert_eq!(slice, &mut [0, 1, 2, 3, 4, 5, 6]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 1, 1, 2, 2, 3]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 0, 1, 1, 1, 2]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 0, 0, 1, 1, 1]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 0, 0, 0, 1, 1]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 0, 0, 0, 0, 1]);
    }
