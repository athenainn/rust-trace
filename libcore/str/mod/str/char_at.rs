...
mod tests {
	...
	let result: char = x.char_at(0);
	assert_eq!(result, '精');

	let result: char = x.char_at(3);
	assert_eq!(result, '誠');
    }
	...
	let _: char = x.char_at(1); // panicked at 'assertion failed: w != 0'
    }
    ...
    fn char_at_test3() {
	let x: &str = "銹";
	let _: char = x.char_at(2); // panicked at 'assertion failed: w != 0'
    }
    ...
    fn char_at_test4() {
	let x: &str = "銹";
	let _: char = x.char_at(3); // panicked at 'index out of bounds: the len is 3 but the index is 3'
    }
}
