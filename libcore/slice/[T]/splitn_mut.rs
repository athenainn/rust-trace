...
mod tests {
	...
	let n: usize = 0;
	let pred: P = P;
	let mut splitn: SplitNMut<T, P> = slice.splitn_mut::<P>(n, pred);

	assert_eq!(splitn.next(), None::<&mut [T]>);
    }
	...
	{
	    let mut splitn_mut: SplitNMut<T, P> = slice.splitn_mut::<P>(n, pred);
	    let len: usize = splitn_mut.size_hint().1.unwrap();

	    for _ in 0..len {
		let x: Option<&mut [T]> = splitn_mut.next();
		match x {
		    Some(v) => {
			for i in 0..v.len() {
			    v[i] += 10;
	...
	assert_eq!(slice, &mut [21, 12, 22, 23, 24, 25, 26]);
    }
	...
	assert_eq!(slice, &mut [21, 12, 12, 23, 24, 25, 26]);
    }
