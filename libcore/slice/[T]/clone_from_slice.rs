...
mod tests {
    ...
    #[test]
    fn clone_from_slice_test1() {
	let slice: &mut [T] = &mut [1, 2, 3, 4, 5, 6, 7, 8];
	let src: &[T] = &[11, 12, 13];
	let clone_from_slice: usize = slice.clone_from_slice(src);

	assert_eq!(clone_from_slice, 3);
	assert_eq!(slice, &[11, 12, 13, 4, 5, 6, 7, 8]);
    }
}
