...
mod tests {
    ...
    #[test]
    fn index_mut_test2() {
	let slice1: &mut [T] = &mut [1, 2, 3, 4, 5, 6];
	let start: usize = 1;
	let end: usize = 5;
	let range: Range<usize> = start..end;

	{
	    let slice2: &mut [T] = &mut slice1[range];

	    for index in 0..slice2.len() {
		slice2[index] += 10;
	    }
	}

	assert_eq!(slice1, &mut [1, 12, 13, 14, 15, 6]);
    }
	...
	let slice1: &mut [T] = &mut [1, 2, 3, 4, 5, 6];
	let end: usize = 5;
	let rangeto: RangeTo<usize> = ..end;

	{
	    let slice2: &mut [T] = &mut slice1[rangeto];
	...
	assert_eq!(slice1, &mut [11, 12, 13, 14, 15, 6]);
    }
	...
	let slice1: &mut [T] = &mut [1, 2, 3, 4, 5, 6];
	let start: usize = 1;
	let rangefrom: RangeFrom<usize> = start..;

	{
	    let slice2: &mut [T] = &mut slice1[rangefrom];
	...
	assert_eq!(slice1, &mut [1, 12, 13, 14, 15, 16]);
    }
	...
	{
	    let slice2: &mut [T] = &mut slice1[rangefull];
	...
	assert_eq!(slice1, &mut [11, 12, 13, 14, 15, 16]);
    }
}
