...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xsub_rel;
    ...
    macro_rules! atomic_xsub_rel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xsub_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xsub_rel_test1() {
	atomic_xsub_rel_test!( 500, 68, 432 );
    }
}
