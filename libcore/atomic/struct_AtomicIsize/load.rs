...
mod tests {
    ...
    #[test]
    fn load_test1() {
	let atomicisize: AtomicIsize = AtomicIsize::default();
	...
	let result: isize = data.load(Acquire);
	assert_eq!(result, 68);
    }
	...
	let result: isize = data.load(SeqCst);
	assert_eq!(result, 68);
    }
    ...
    fn load_test4() {
	let atomicisize: AtomicIsize = AtomicIsize::default();
	let _: isize = atomicisize.load(Release); // panicked at 'there is no such thing as a release load'
    }
    ...
    fn load_test5() {
	let atomicisize: AtomicIsize = AtomicIsize::default();
	let _: isize = atomicisize.load(AcqRel); // panicked at 'there is no such thing as an acquire/release load'
    }
}
