...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xchg_relaxed;
    ...
    macro_rules! atomic_xchg_relaxed_test {
	($init:expr, $value:expr) => ({
		...
		let src: T = $value;
		let result: T = unsafe { atomic_xchg_relaxed::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xchg_relaxed_test1() {
	atomic_xchg_relaxed_test!(  68, 500 );
    }
}
