...
mod tests {
    ...
    macro_rules! atomic_load_acq_test {
	($value:expr) => ({
	    let value: T = $value;
		...
		let result: T = unsafe { atomic_load_acq::<T>(src) };
	    ...
	    assert_eq!(unsafe { atomic_load_acq::<T>(ptr) }, $value);
	})
    }

    #[test]
    fn atomic_load_acq_test1() {
	atomic_load_acq_test!( 68 );
    }
}
