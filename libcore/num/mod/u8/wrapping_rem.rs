...
mod tests {
	...
	($value:expr, $rhs:expr, $result:expr) => ({
	    let x: u8 = $value;
	    let rhs: u8  = $rhs;
	    let result: u8 = x.wrapping_rem(rhs);
	...
	let x: u8 = 68;
	let rhs: u8  = 0;
	let _: u8 = x.wrapping_rem(rhs); // panicked at 'attempted remainder with a divisor of zero'
    }

    #[test]
    fn wrapping_rem_test2() {
	wrapping_rem_test!( 0xff, 0x01, 0x00 );
	wrapping_rem_test!( 0xff, 0x02, 0x01 );
	wrapping_rem_test!( 0xff, 0x03, 0x00 );
	wrapping_rem_test!( 0xff, 0x04, 0x03 );
    }
}
