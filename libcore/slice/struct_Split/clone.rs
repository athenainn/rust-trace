...
mod tests {
	...
	let pred: P = P;
	let mut split: Split<T, P> = slice.split::<P>(pred);
	let mut clone: Split<T, P> = split.clone();

	assert_eq!(split.next(), Some::<&[T]>(&[11]));
	assert_eq!(split.next(), Some::<&[T]>(&[]));
	assert_eq!(split.next(), Some::<&[T]>(&[13, 14]));
	assert_eq!(split.next(), Some::<&[T]>(&[16]));
	assert_eq!(split.next(), None::<&[T]>);

	assert_eq!(clone.next(), Some::<&[T]>(&[11]));
	assert_eq!(clone.next(), Some::<&[T]>(&[]));
	assert_eq!(clone.next(), Some::<&[T]>(&[13, 14]));
