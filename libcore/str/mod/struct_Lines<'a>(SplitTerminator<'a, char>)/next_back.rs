...
mod tests {
	...
	let x: &str = "古人學問無遺力\n\
		       ...
		       紙上得來終覺淺\n\
		       絕知此事要躬行";
	let mut lines: Lines = x.lines();

	assert_eq!(lines.next_back(), Some::<&str>("絕知此事要躬行"));
	assert_eq!(lines.next_back(), Some::<&str>("紙上得來終覺淺"));
	assert_eq!(lines.next_back(), Some::<&str>("少壯功夫老始成"));
	assert_eq!(lines.next_back(), Some::<&str>("古人學問無遺力"));
	assert_eq!(lines.next_back(), None::<&str>);
    }
}
