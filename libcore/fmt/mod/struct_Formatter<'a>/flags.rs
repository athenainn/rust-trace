...
mod tests {
	...
	fn fmt(&self, fmt: &mut Formatter) -> Result {
	    let flags: u32 = fmt.flags();

	    fmt.write_fmt(format_args!("{:#x}", flags))
	}
    }
	...
	assert_eq!(output, "0x0".to_string());
    }

    #[test]
    fn flags_test2() {
	let a: A<T> = A::new();
	let output: String = format!("{:+}", a); // format with sign plus

	assert_eq!(output, "0x1".to_string());
    }

    #[test]
    fn flags_test3() {
	let a: A<T> = A::new();
	let output: String = format!("{:-}", a); // format with sign minus

	assert_eq!(output, "0x2".to_string());
    }

    #[test]
    fn flags_test4() {
	let a: A<T> = A::new();
	let output: String = format!("{:#}", a); // format with alternate

	assert_eq!(output, "0x4".to_string());
    }

    #[test]
    fn flags_test5() {
	let a: A<T> = A::new();
	let output: String = format!("{:0}", a); // format with sign-aware zero padding

	assert_eq!(output, "0x8".to_string());
    }
}
