...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xsub_acqrel;
    ...
    macro_rules! atomic_xsub_acqrel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xsub_acqrel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xsub_acqrel_test1() {
	atomic_xsub_acqrel_test!( 500, 68, 432 );
    }
}
