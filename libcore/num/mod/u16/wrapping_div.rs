...
mod tests {
	...
	($value:expr, $rhs:expr, $result:expr) => ({
	    let x: u16 = $value;
	    let rhs: u16  = $rhs;
	    let result: u16 = x.wrapping_div(rhs);
	...
	let x: u16 = 68;
	let rhs: u16  = 0;
	let _: u16 = x.wrapping_div(rhs); // panicked at 'attempted to divide by zero'
    }

    #[test]
    fn wrapping_div_test2() {
	wrapping_div_test!( 0xffff, 0x0001, 0xffff );
	wrapping_div_test!( 0xffff, 0x0002, 0x7fff );
	wrapping_div_test!( 0xffff, 0x0003, 0x5555 );
    }
}
