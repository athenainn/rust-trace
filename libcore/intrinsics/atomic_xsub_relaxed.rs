...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xsub_relaxed;
    ...
    macro_rules! atomic_xsub_relaxed_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xsub_relaxed::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xsub_relaxed_test1() {
	atomic_xsub_relaxed_test!( 500, 68, 432 );
    }
}
