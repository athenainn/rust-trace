...
mod tests {
	...
	let bits: usize = 16;
	let mul_pow2: &mut Big8x3 = big.mul_pow2(bits);

	let result: String = format!("{:?}", mul_pow2);
	assert_eq!(result, "0x68_00_00".to_string());
    }
	...
	let mut big: Big8x3 = Big8x3::from_small(v);
	let bits: usize = 24;
	let _: &mut Big8x3 = big.mul_pow2(bits); // panicked at 'assertion failed: digits < 3'
    }
}
