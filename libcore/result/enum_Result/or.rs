...
mod tests {
	...
	let res: Result<T, F> = Err::<T, F>("late error");
	let result: Result<T, F> = x.or::<F>(res);
	...
	assert_eq!(result, Err::<T, F>("late error"));
    }

    #[test]
    fn or_test4() {
	let x: Result<T, E> = Ok::<T, E>(2);
	let res: Result<T, F> = Ok::<T, F>(100);
	let result: Result<T, F> = x.or::<F>(res);
