...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xadd_acq;
    ...
    macro_rules! atomic_xadd_acq_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xadd_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xadd_acq_test1() {
	atomic_xadd_acq_test!( 68, 500, 568 );
    }
}
