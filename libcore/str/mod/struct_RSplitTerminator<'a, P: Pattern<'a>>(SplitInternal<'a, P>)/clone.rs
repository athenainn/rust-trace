...
mod tests {
	...
	let pat: P = "葡萄皮";
	let mut rsplit_terminator: RSplitTerminator<P> = x.rsplit_terminator::<P>(pat);
	let mut clone: RSplitTerminator<P> = rsplit_terminator.clone();

	assert_eq!(rsplit_terminator.next(), Some::<&str>("不吃葡萄倒吐"));
	assert_eq!(rsplit_terminator.next(), Some::<&str>("吃葡萄不吐"));
	assert_eq!(rsplit_terminator.next(), None::<&str>);

	assert_eq!(clone.next(), Some::<&str>("不吃葡萄倒吐"));
	assert_eq!(clone.next(), Some::<&str>("吃葡萄不吐"));
	assert_eq!(clone.next(), None::<&str>);
    }
	...
	assert_eq!(rsplit_terminator.next(), Some::<&str>("不吃葡萄倒吐葡萄"));
	assert_eq!(rsplit_terminator.next(), Some::<&str>("吃葡萄不吐葡萄"));
	assert_eq!(rsplit_terminator.next(), None::<&str>);

	assert_eq!(clone.next(), Some::<&str>("不吃葡萄倒吐葡萄"));
	assert_eq!(clone.next(), Some::<&str>("吃葡萄不吐葡萄"));
	assert_eq!(clone.next(), None::<&str>);
    }
	...
	assert_eq!(rsplit_terminator.next(), Some::<&str>("葡萄倒吐葡萄"));
	assert_eq!(rsplit_terminator.next(), Some::<&str>("不"));
	assert_eq!(rsplit_terminator.next(), Some::<&str>("葡萄不吐葡萄"));
	assert_eq!(rsplit_terminator.next(), Some::<&str>(""));
	assert_eq!(rsplit_terminator.next(), None::<&str>);

	assert_eq!(clone.next(), Some::<&str>("葡萄倒吐葡萄"));
	assert_eq!(clone.next(), Some::<&str>("不"));
	assert_eq!(clone.next(), Some::<&str>("葡萄不吐葡萄"));
	assert_eq!(clone.next(), Some::<&str>(""));
	assert_eq!(clone.next(), None::<&str>);
    }

    #[test]
    fn clone_test4() {
	type P = F; // P: Pattern<'a> + Clone
