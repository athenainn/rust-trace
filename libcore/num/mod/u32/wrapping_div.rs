...
mod tests {
	...
	($value:expr, $rhs:expr, $result:expr) => ({
	    let x: u32 = $value;
	    let rhs: u32  = $rhs;
	    let result: u32 = x.wrapping_div(rhs);
	...
	let x: u32 = 68;
	let rhs: u32  = 0;
	let _: u32 = x.wrapping_div(rhs); // panicked at 'attempted to divide by zero'
    }

    #[test]
    fn wrapping_div_test2() {
	wrapping_div_test!( 0xffffffff, 0x00000001, 0xffffffff );
	wrapping_div_test!( 0xffffffff, 0x00000002, 0x7fffffff );
	wrapping_div_test!( 0xffffffff, 0x00000003, 0x55555555 );
    }
}
