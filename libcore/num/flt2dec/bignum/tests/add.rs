...
mod tests {
    ...
    fn add_test1() {
	let v1: u8 = 0x68;
	let mut big: Big8x3 = Big8x3::from_small(v1);

	let v2: u8 = 0x99;
	let other: Big8x3 = Big8x3::from_small(v2);

	let add: &Big8x3 = big.add(&other);

	let result: String = format!("{:?}", add);
	assert_eq!(result, "0x1_01".to_string());
    }
	...
	let other: &[u8] = &[ 0xff, 0xff, 0xff ];
	let mut big: &mut Big8x3 = big.mul_digits(other);

	let v2: u8 = 0x01;
	let other: Big8x3 = Big8x3::from_small(v2);

	let _: &Big8x3 = big.add(&other); // panicked at 'index out of bounds: the len is 3 but the index is 3'
    }
}
