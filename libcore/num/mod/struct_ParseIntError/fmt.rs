...
mod tests {
	...
	let message: String = format!("{}", parseinterror);

	assert_eq!(message, "cannot parse integer from empty string");
    }
	...
	assert_eq!(message, "number too large to fit in target type");
    }
	...
	assert_eq!(message, "number too small to fit in target type");
    }
}
