...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_umin_rel;
    ...
    macro_rules! atomic_umin_rel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_umin_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_umin_rel_test1() {
	atomic_umin_rel_test!( !0, 0, 0 );
    }
}
