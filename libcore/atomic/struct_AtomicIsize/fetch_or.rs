...
mod tests {
	...
	($init:expr, $val:expr, $order:ident, $result:expr) => ({
		...
		let result: isize = clone.fetch_or(val, $order);
		assert_eq!(result, $init);
	    });
    ...
    #[test]
    fn fetch_or_test1() {
	fetch_or_test!( 0, 0, Relaxed, 0 );
	fetch_or_test!( 0, 0, Release, 0 );
	fetch_or_test!( 0, 0, Acquire, 0 );
	fetch_or_test!( 0, 0, AcqRel, 0 );
	fetch_or_test!( 0, 0, SeqCst, 0 );
    }
