...
mod tests {
    ...
    #[test]
    fn get_unchecked_test1() {
	let slice: &[T] = &[11, 12, 12, 13, 14, 15, 16];

	for i in 0..slice.len() {
	    let get_unchecked: &T = unsafe { slice.get_unchecked(i) };
	    assert_eq!(*get_unchecked, slice[i]);
	}
    }

    #[test]
    fn get_unchecked_test2() {
	let slice1: &[T] = &[];
	let slice2: &[T] = &[68];

	unsafe {
	    assert_eq!(slice1.get_unchecked(0), &0);
	    assert_eq!(slice1.get_unchecked(1), &68);
	    assert_eq!(slice2.get_unchecked(0), &68);
	}
    }
}
