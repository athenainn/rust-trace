...
mod tests {
    ...
    #[test]
    fn rsplitn_mut_test1() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 0;
	let pred: P = P;
	let mut rsplitn_mut: RSplitNMut<T, P> = slice.rsplitn_mut::<P>(n, pred);

	assert_eq!(rsplitn_mut.next(), None::<&mut [T]>);
    }

    #[test]
    fn rsplitn_mut_test2() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 1;
	let pred: P = P;
	...
	{
	    let mut rsplitn_mut: RSplitNMut<T, P> = slice.rsplitn_mut::<P>(n, pred);
	    let len: usize = rsplitn_mut.size_hint().1.unwrap();

	    for _ in 0..len {
		let x: Option<&mut [T]> = rsplitn_mut.next();
		match x {
		    Some(v) => {
			for i in 0..v.len() {
			    v[i] += 10;
	...
	assert_eq!(slice, &mut [21, 22, 22, 23, 24, 25, 26]);
    }

    #[test]
    fn rsplitn_mut_test3() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 2;
	let pred: P = P;
	...
	assert_eq!(slice, &mut [21, 22, 22, 23, 24, 15, 26]);
    }

    #[test]
    fn rsplitn_mut_test4() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 3;
	let pred: P = P;
	...
	assert_eq!(slice, &mut [21, 22, 12, 23, 24, 15, 26]);
    }

    #[test]
    fn rsplitn_mut_test5() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 4;
	let pred: P = P;
	...
	assert_eq!(slice, &mut [21, 12, 12, 23, 24, 15, 26]);
    }

    #[test]
    fn rsplitn_mut_test6() {
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let n: usize = 5;
	let pred: P = P;
