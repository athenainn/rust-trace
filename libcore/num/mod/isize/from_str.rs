...
mod tests {
	...
	let result: Result<isize, ParseIntError> = isize::from_str(src);

	assert_eq!(result, Ok::<isize, ParseIntError>(68));
    }
    ...
    #[test]
    fn from_str_test5() {
	let src: &'static str = match cfg!(target_pointer_width = "64") {
    ...
    fn from_str_test6() {
	let src: &'static str = match cfg!(target_pointer_width = "64") {
	    true => "-9223372036854775809",
	    false => "-2147483649"
	};

	let result: Result<isize, ParseIntError> = isize::from_str(src);

	assert_eq!(result.unwrap_err().__description(), "number too small to fit in target type");
    }
}
