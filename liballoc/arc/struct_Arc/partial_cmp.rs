...
mod tests {
	...
	($T:ty) => ({
	    {
		let arc: Arc<$T> = Arc::<$T>::new(68);
		let other: Arc<$T> = Arc::<$T>::new(500);
		let result: Option<Ordering> = arc.partial_cmp(&other);
