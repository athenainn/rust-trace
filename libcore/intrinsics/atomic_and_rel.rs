...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_and_rel;
    ...
    macro_rules! atomic_and_rel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_and_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_and_rel_test1() {
	atomic_and_rel_test!( 0xff00, 0x0a0a, 0x0a00 );
    }
}
