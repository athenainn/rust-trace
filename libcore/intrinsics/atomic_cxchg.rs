...
mod tests {
	...
	($init:expr, $old:expr, $new:expr, $result:expr) => ({
		...
		let old: T = $old;
		let src: T = $new;
		let result: T = unsafe { atomic_cxchg::<T>(dst, old, src) };
    ...
    #[test]
    fn atomic_cxchg_test1() {
	atomic_cxchg_test!(  0,  0, 500, 500 );
	atomic_cxchg_test!(  0, 68, 500,   0 );
	atomic_cxchg_test!( 68,  0, 500,  68 );
	atomic_cxchg_test!( 68, 68, 500, 500 );
    }
}
