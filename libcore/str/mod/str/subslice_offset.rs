...
mod tests {
	...
	let pat: P = "葡萄皮";
	let mut matches: Matches<P> = x.matches(pat);

	let next: Option<&str> = matches.next();
	let inner: &str = next.unwrap();
	assert_eq!(inner, "葡萄皮");
	let result: usize = x.subslice_offset(inner);
	assert_eq!(result, 15);
	...
	let x: &str = "吃葡萄不吐葡萄皮不吃葡萄倒吐葡萄皮";
	let pat: F = F { c: '皮' };
	let mut matches: Matches<_> = x.matches(pat); // P: Pattern<'a>
