...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_max_rel;
    ...
    macro_rules! atomic_max_rel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_max_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_max_rel_test1() {
	atomic_max_rel_test!( !0, 0, 0 );
    }
}
