...
mod tests {
	...
	($init:expr, $current:expr, $new:expr, $order:ident, $result:expr) => ({
	    let atomicusize: AtomicUsize = AtomicUsize::new($init);
		...
		let result: usize = clone.compare_and_swap(current, new, $order);
		assert_eq!(result, $init);
	    });
	...
	compare_and_swap_test!( 68, 0, 0, Relaxed, 68 );
	compare_and_swap_test!( 68, 0, 0, Release, 68 );
	compare_and_swap_test!( 68, 0, 0, Acquire, 68 );
	compare_and_swap_test!( 68, 0, 0, AcqRel, 68 );
	compare_and_swap_test!( 68, 0, 0, SeqCst, 68 );
    }
