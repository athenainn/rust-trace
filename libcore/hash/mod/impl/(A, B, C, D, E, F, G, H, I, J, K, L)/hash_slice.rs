...
mod tests {
	...
	let data: &[(A, B, C, D, E, F, G, H, I, J, K, L)] = &[(
	    ...
	    I::default(), J::default(), K::default(), L::default()
	)];
	let mut state: S = <S>::new();
	let finish: u64 = state.finish();
	assert_eq!(finish, 0x1e924B9d737700d7);

	<(A, B, C, D, E, F, G, H, I, J, K, L)>::hash_slice::<S>(data, &mut state);

	let finish: u64 = state.finish();
	assert_eq!(finish, 0x5791e8712ef38211);
    }
}
