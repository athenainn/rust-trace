...
mod tests {
	...
	atomicbool.store(val, Acquire); // panicked at 'there is no such thing as an acquire store'
    }
	...
	atomicbool.store(val, AcqRel); // panicked at 'there is no such thing as an acquire/release store'
    }
}
