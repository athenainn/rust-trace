...
mod tests {
    ...
    #[test]
    fn div_rem_small_test1() {
	let v: Digit32 = 0x00000001;
	let mut big: Big32x36 = Big32x36::from_small(v);
	let other: &[Digit32] = &[ 0x9abcdef0, 0x12345678 ];
	let mul_digits: &mut Big32x36 = big.mul_digits(other);

	let other: Digit32 = 256;
	let (quotient, remainder): (&mut Big32x36, Digit32) = mul_digits.div_rem_small(other);

	let result: String = format!("{:?}", quotient);
	assert_eq!(result, "0x123456_789abcde".to_string());
	assert_eq!(remainder, 240);
    }

    #[test]
    #[should_panic]
    fn div_rem_small_test2() {
	let v: Digit32 = 0x00000001;
	let mut big: Big32x36 = Big32x36::from_small(v);
	let other: &[Digit32] = &[ 0x9abcdef0, 0x12345678 ];
	let mul_digits: &mut Big32x36 = big.mul_digits(other);
	...
	let other: Digit32 = 0;
	let _: (&mut Big32x36, Digit32) = mul_digits.div_rem_small(other); // panicked at 'assertion failed: other > 0'
    }
}
