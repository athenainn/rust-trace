...
mod tests {
    ...
    #[test]
    fn mul_small_test1() {
	let v: Digit32 = 0x12345678;
	let mut big: Big32x36 = Big32x36::from_small(v);
	let other: Digit32 = 0xfedcba98;
	let mul_small: &mut Big32x36 = big.mul_small(other);

	let result: String = format!("{:?}", mul_small);
	assert_eq!(result, "0x121fa00a_35068740".to_string());
    }
	...
	let big: &mut Big32x36 = big.mul_digits(other);

	let v: Digit32 = 0x00000002;
	let _: &mut Big32x36 = big.mul_small(v); // panicked at 'index out of bounds: the len is 36 but the index is 36'
    }
}
