...
mod tests {
	...
	let next_permutation: bool = slice.next_permutation();

	assert_eq!(next_permutation, false);
	assert_eq!(slice, &mut [2, 2, 1, 1]);
    }
	...
	assert_eq!(slice.next_permutation(), true);
	...
	assert_eq!(slice.next_permutation(), false);
	assert_eq!(slice, &mut [2, 2, 1, 1]);
    }
}
