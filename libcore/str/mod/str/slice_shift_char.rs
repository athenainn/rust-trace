...
mod tests {
    ...
    #[test]
    fn slice_shift_char_test1() {
	let x: &str = "天地泰";

	let result: Option<(char, &str)> = x.slice_shift_char();
	let (c, remain): (char, &str) = result.unwrap();
	assert_eq!(c, '天');
	assert_eq!(remain, "地泰");

	let result: Option<(char, &str)> = remain.slice_shift_char();
	let (c, remain): (char, &str) = result.unwrap();
	assert_eq!(c, '地');
	assert_eq!(remain, "泰");
