...
mod tests {
	...
	assert_eq!(lines.next(), Some::<&str>("古人學問無遺力"));
	assert_eq!(lines.next(), Some::<&str>("少壯功夫老始成"));
	assert_eq!(lines.next(), Some::<&str>("紙上得來終覺淺"));
	assert_eq!(lines.next(), Some::<&str>("絕知此事要躬行"));
	assert_eq!(lines.next(), None::<&str>);
    }
}
