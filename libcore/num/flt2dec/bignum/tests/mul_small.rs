...
mod tests {
	...
	let other: u8 = 0x99;
	let mul_small: &mut Big8x3 = big.mul_small(other);

	let result: String = format!("{:?}", mul_small);
	assert_eq!(result, "0x3e_28".to_string());
    }
	...
	let other: u8 = 0x02;
	let _: &mut Big8x3 = big.mul_small(other); // panicked at 'index out of bounds: the len is 3 but the index is 3'
    }
}
