...
mod tests {
	...
	fn fmt(&self, fmt: &mut Formatter) -> Result {
	    let iter: Iter<(String, T)> = self.0.iter();
	    let map: Map<Iter<(String, T)>, _> = iter.map(|&(ref k, ref v)| (k, v));
	    let mut debugmap: DebugMap = fmt.debug_map();

	    type K<'a> = &'a String;
	    type V<'a> = &'a T;
	    let debugmap_refmut: &mut DebugMap = debugmap.entries::<K, V, _>(map);
	    let result: Result = debugmap_refmut.finish();
    ...
    #[test]
    fn entries_test1() {
	let a: A = A(vec!(("A".to_string(), 68), ("B".to_string(), 500)));
	let output: String = format!("{:?}", a);

	assert_eq!(output, "{\"A\": 68, \"B\": 500}".to_string());
    }
