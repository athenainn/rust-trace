...
mod tests {
	...
	let count: usize = 3;
	let pat: P = "葡萄皮";
	let mut splitn: SplitN<P> = x.splitn::<P>(count, pat);
	let mut clone: SplitN<P> = splitn.clone();

	assert_eq!(splitn.next(), Some::<&str>("吃葡萄不吐"));
	assert_eq!(splitn.next(), Some::<&str>("不吃葡萄倒吐"));
	assert_eq!(splitn.next(), Some::<&str>(""));
	assert_eq!(splitn.next(), None::<&str>);
	...
	assert_eq!(splitn.next(), Some::<&str>("吃葡萄不吐葡萄"));
	assert_eq!(splitn.next(), Some::<&str>("不吃葡萄倒吐葡萄"));
	assert_eq!(splitn.next(), Some::<&str>(""));
	assert_eq!(splitn.next(), None::<&str>);
	...
	assert_eq!(splitn.next(), Some::<&str>("吃葡萄"));
	assert_eq!(splitn.next(), Some::<&str>("吐葡萄"));
	assert_eq!(splitn.next(), Some::<&str>("不吃葡萄倒吐葡萄皮"));
	assert_eq!(splitn.next(), None::<&str>);

	assert_eq!(clone.next(), Some::<&str>("吃葡萄"));
	assert_eq!(clone.next(), Some::<&str>("吐葡萄"));
	assert_eq!(clone.next(), Some::<&str>("不吃葡萄倒吐葡萄皮"));
	assert_eq!(clone.next(), None::<&str>);
    }
	...
	let count: usize = 3;
	let pat: P = P { c: '皮' };
	let mut splitn: SplitN<P> = x.splitn::<P>(count, pat); // P: Pattern<'a> + Clone
	let mut clone: SplitN<P> = splitn.clone();
