...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xsub_acq;
    ...
    macro_rules! atomic_xsub_acq_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xsub_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xsub_acq_test1() {
	atomic_xsub_acq_test!( 500, 68, 432 );
    }
}
