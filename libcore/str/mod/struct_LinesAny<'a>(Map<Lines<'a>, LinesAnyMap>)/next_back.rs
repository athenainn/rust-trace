...
mod tests {
	...
	let x: &str = "古人學問無遺力\r\n\
		       ...
		       紙上得來終覺淺\r\n\
		       絕知此事要躬行";
	let mut lines_any: LinesAny = x.lines_any();

	assert_eq!(lines_any.next_back(), Some::<&str>("絕知此事要躬行"));
	assert_eq!(lines_any.next_back(), Some::<&str>("紙上得來終覺淺"));
	assert_eq!(lines_any.next_back(), Some::<&str>("少壯功夫老始成"));
	assert_eq!(lines_any.next_back(), Some::<&str>("古人學問無遺力"));
	assert_eq!(lines_any.next_back(), None::<&str>);
    }
}
