...
mod tests {
	...
	{
	    let (left, right): (&mut [T], &mut [T]) = slice.split_at_mut(mid);

	    for i in 0..left.len() {
		let x: Option<&mut T> = left.get_mut(i);
		match x {
		    Some(v) => *v += 10,
		    None => assert!(false)
		}
		...
		let x: Option<&mut T> = right.get_mut(i);
		match x {
		    Some(v) => *v += 20,
		    None => assert!(false)
		}
	    ...
	    assert_eq!(left, &[11, 12, 13]);
	    assert_eq!(right, &[24, 25, 26]);
	}

	assert_eq!(slice, &mut [11, 12, 13, 24, 25, 26]);
    }
}
