...
mod tests {
	...
	let other: &[Digit32] = &[ 0x00000000, 0x00000001 ];
	let mul_digits: &mut Big32x36 = big.mul_digits(other);

	let result: String = format!("{:?}", mul_digits);
	assert_eq!(result, "0x12345678_00000000".to_string());
    }
	...
	let _: &mut Big32x36 = big.mul_digits(other); // panicked at 'assertion failed: index < self.len()'
    }
}
