...
mod tests {
	...
	let result: usize = data.load(Acquire);
	assert_eq!(result, 68);
    }
	...
	let result: usize = data.load(SeqCst);
	assert_eq!(result, 68);
    }
    ...
    fn load_test4() {
	let atomicusize: AtomicUsize = AtomicUsize::default();
	let _: usize = atomicusize.load(Release); // panicked at 'there is no such thing as a release load'
    }
    ...
    fn load_test5() {
	let atomicusize: AtomicUsize = AtomicUsize::default();
	let _: usize = atomicusize.load(AcqRel); // panicked at 'there is no such thing as an acquire/release load'
    }
}
