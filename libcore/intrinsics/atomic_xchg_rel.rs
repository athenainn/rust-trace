...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xchg_rel;
    ...
    macro_rules! atomic_xchg_rel_test {
	($init:expr, $value:expr) => ({
		...
		let src: T = $value;
		let result: T = unsafe { atomic_xchg_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xchg_rel_test1() {
	atomic_xchg_rel_test!(  68, 500 );
    }
}
