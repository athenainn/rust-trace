...
mod tests {
	...
	let size: usize = 0;
	let _: ChunksMut<T> = slice.chunks_mut(size); // panicked at 'assertion failed: chunk_size > 0'
    }
	...
	{
	    let mut chunks_mut: ChunksMut<T> = slice.chunks_mut(size);
	    let len: usize = chunks_mut.size_hint().1.unwrap();

	    for i in 0..len {
		let x: Option<&mut [T]> = chunks_mut.next_back();
		match x {
		    Some(v) => {
			for j in 0..v.len() {
			    v[j] = i as T;
	...
	assert_eq!(slice, &mut [6, 5, 4, 3, 2, 1, 0]);
    }
	...
	assert_eq!(slice, &mut [3, 3, 2, 2, 1, 1, 0]);
    }
	...
	assert_eq!(slice, &mut [2, 2, 2, 1, 1, 1, 0]);
    }
	...
	assert_eq!(slice, &mut [1, 1, 1, 1, 0, 0, 0]);
    }
	...
	assert_eq!(slice, &mut [1, 1, 1, 1, 1, 0, 0]);
    }
	...
	assert_eq!(slice, &mut [1, 1, 1, 1, 1, 1, 0]);
    }
	...
	assert_eq!(slice, &mut [0, 0, 0, 0, 0, 0, 0]);
    }
