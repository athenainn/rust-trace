...
mod tests {
	...
	// `<&str as Pattern<'a>>::Searcher` is not a `DoubleEndedSearcher<'a>`
    }
	...
	let pat: P = '皮';
	let mut result: RSplitTerminator<P> = x.rsplit_terminator::<P>(pat);

	assert_eq!(result.next_back(), Some::<&str>("吃葡萄不吐葡萄"));
	assert_eq!(result.next_back(), Some::<&str>("不吃葡萄倒吐葡萄"));
	assert_eq!(result.next_back(), None::<&str>);
    }
	...
	assert_eq!(result.next_back(), Some::<&str>(""));
	assert_eq!(result.next_back(), Some::<&str>("葡萄不吐葡萄"));
	assert_eq!(result.next_back(), Some::<&str>("不"));
	assert_eq!(result.next_back(), Some::<&str>("葡萄倒吐葡萄"));
	assert_eq!(result.next_back(), None::<&str>);
    }
