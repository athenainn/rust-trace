...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_max_relaxed;
    ...
    macro_rules! atomic_max_relaxed_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_max_relaxed::<T>(dst, src) };
    ...
    #[test]
    fn atomic_max_relaxed_test1() {
	atomic_max_relaxed_test!( !0, 0, 0 );
    }
}
