...
mod tests {
	...
	assert_eq!(split.next_back(), Some::<&[T]>(&[16]));
	assert_eq!(split.next_back(), Some::<&[T]>(&[13, 14]));
	assert_eq!(split.next_back(), Some::<&[T]>(&[]));
	assert_eq!(split.next_back(), Some::<&[T]>(&[11]));
	assert_eq!(split.next_back(), None::<&[T]>);
    }
}
