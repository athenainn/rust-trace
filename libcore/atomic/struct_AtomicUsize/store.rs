...
mod tests {
	...
	atomicusize.store(val, Acquire); // panicked at 'there is no such thing as an acquire store'
    }
	...
	atomicusize.store(val, AcqRel); // panicked at 'there is no such thing as an acquire/release store'
    }
}
