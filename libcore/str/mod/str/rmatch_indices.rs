...
mod tests {
    ...
    #[test]
    fn rmatch_indices_test1() {
	type P<'a> = &'a str; // P: Pattern<'a>

	let x: &str = "吃葡萄不吐葡萄皮不吃葡萄倒吐葡萄皮";
	let pat: P = "葡萄皮";
	let mut result: RMatchIndices<P> = x.rmatch_indices::<P>(pat);

	assert_eq!(result.next(), Some::<(usize, usize)>((42, 51)));
	assert_eq!(result.next(), Some::<(usize, usize)>((15, 24)));
	assert_eq!(result.next(), None::<(usize, usize)>);
    }

    #[test]
    fn rmatch_indices_test2() {
	...
	assert_eq!(result.next(), Some::<(usize, usize)>((48, 51)));
	assert_eq!(result.next(), Some::<(usize, usize)>((21, 24)));
	assert_eq!(result.next(), None::<(usize, usize)>);
    }

    #[test]
    fn rmatch_indices_test3() {
	...
	assert_eq!(result.next(), Some::<(usize, usize)>((48, 51)));
	assert_eq!(result.next(), Some::<(usize, usize)>((24, 27)));
	assert_eq!(result.next(), Some::<(usize, usize)>((21, 24)));
	assert_eq!(result.next(), Some::<(usize, usize)>((9, 12)));
	assert_eq!(result.next(), None::<(usize, usize)>);
    }

    #[test]
    fn rmatch_indices_test4() {
