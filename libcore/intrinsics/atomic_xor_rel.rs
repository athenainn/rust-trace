...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xor_rel;
    ...
    macro_rules! atomic_xor_rel_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xor_rel::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xor_rel_test1() {
	atomic_xor_rel_test!( 0xff00, 0x0a0a, 0xf50a );
    }
}
