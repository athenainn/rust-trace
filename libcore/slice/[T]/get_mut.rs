...
mod tests {
	...
	assert_eq!(slice.get_mut(0), None::<&mut T>);
	assert_eq!(slice.get_mut(1), None::<&mut T>);
    }
	...
	for i in 0..slice.len() {
	    let get_mut: Option<&mut T> = slice.get_mut(i);
	    match get_mut {
		Some(v) => *v += 10,
		None => assert!(false)
	    }
	...
	assert_eq!(slice.get_mut(len), None::<&mut T>);
    }
}
