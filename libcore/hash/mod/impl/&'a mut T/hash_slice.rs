...
mod tests {
    ...
    type T = i32;	// T: ?Size + Hash
    type H = SipHasher; // H: Hasher

    #[test]
    fn hash_slice_test1() {
	let data: &[&mut T] = &[
	    &mut 0x01, &mut 0x02, &mut 0x03, &mut 0x04,
	    &mut 0x05, &mut 0x06, &mut 0x07, &mut 0x08
	];
	let mut state: H = <H>::new();
	let finish: u64 = state.finish();
	assert_eq!(finish, 0x1e924b9d737700d7);

	<&mut T>::hash_slice::<H>(data, &mut state);

	let finish: u64 = state.finish();
	assert_eq!(finish, 0x77996ca12191232f);
    }
}
