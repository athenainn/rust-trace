...
mod tests {
	...
	let p: *mut T = &mut 68;
	let a: A<T> = A::<T>::new(p);
	let _: *mut T = a.ptr.load(Release); // panicked at 'there is no such thing as a release load'
    }
	...
	let p: *mut T = &mut 68;
	let a: A<T> = A::<T>::new(p);
	let _: *mut T = a.ptr.load(AcqRel); // panicked at 'there is no such thing as an acquire/release load'
    }
}
