...
mod tests {
    ...
    #[test]
    fn prev_permutation_test1() {
	let slice: &mut [T] = &mut [1, 1, 2, 2];
	let prev_permutation: bool = slice.prev_permutation();

	assert_eq!(prev_permutation, false);
	assert_eq!(slice, &mut [1, 1, 2, 2]);
    }

    #[test]
    fn prev_permutation_test2() {
	let slice: &mut [T] = &mut [2, 2, 1, 1];

	assert_eq!(slice.prev_permutation(), true);
	assert_eq!(slice, &mut [2, 1, 2, 1]);

	assert_eq!(slice.prev_permutation(), true);
	assert_eq!(slice, &mut [2, 1, 1, 2]);

	assert_eq!(slice.prev_permutation(), true);
	assert_eq!(slice, &mut [1, 2, 2, 1]);

	assert_eq!(slice.prev_permutation(), true);
	assert_eq!(slice, &mut [1, 2, 1, 2]);

	assert_eq!(slice.prev_permutation(), true);
	assert_eq!(slice, &mut [1, 1, 2, 2]);

	assert_eq!(slice.prev_permutation(), false);
	assert_eq!(slice, &mut [1, 1, 2, 2]);
    }
}
