...
mod tests {
	...
	let char_indices: CharIndices = x.char_indices();
	let (lower, upper): (usize, Option<usize>) = char_indices.size_hint();

	assert_eq!(lower, 9); // (36 + 3) / 4
	assert_eq!(upper, Some::<usize>(36));
    }
	...
	let (lower, upper): (usize, Option<usize>) = char_indices.size_hint();
	assert_eq!(lower, 9); // (33 + 3) / 4
	assert_eq!(upper, Some::<usize>(33));
    }
	...
	let (lower, upper): (usize, Option<usize>) = char_indices.size_hint();
	assert_eq!(lower, 8); // (30 + 3) / 4
	assert_eq!(upper, Some::<usize>(30));
    }
}
