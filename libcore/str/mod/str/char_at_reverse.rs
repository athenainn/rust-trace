...
mod tests {
    ...
    #[test]
    fn char_at_reverse_test1() {
	let x: &str = "精誠";

	assert_eq!(x.len(), 6);

	for start in 0..4 {
	    let result: char = x.char_at_reverse(start);
	    assert_eq!(result, '精');
	}
    ...
    #[test]
    #[should_panic]
    fn char_at_reverse_test2() {
	let x: &str = "精誠";
	let _: char = x.char_at_reverse(7); // panicked at 'index out of bounds: the len is 6 but the index is 6'
    }
}
