...
mod tests {
	...
	let pred: P = P;
	let split_mut: SplitMut<T, P> = slice.split_mut::<P>(pred);
	let (lower, upper): (usize, Option<usize>) = split_mut.size_hint();
	...
	let slice: &mut [T] = &mut [11, 12, 12, 13, 14, 15, 16];
	let pred: P = P;
	let mut split_mut: SplitMut<T, P> = slice.split_mut::<P>(pred);

	assert_eq!(split_mut.next(), Some::<&mut [T]>(&mut [11]));
