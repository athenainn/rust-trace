...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_umin_acq;
    ...
    macro_rules! atomic_umin_acq_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_umin_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_umin_acq_test1() {
	atomic_umin_acq_test!( !0, 0, 0 );
    }
}
