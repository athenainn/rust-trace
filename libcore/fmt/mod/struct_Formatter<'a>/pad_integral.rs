...
mod tests {
    ...
    impl Display for A<u8> {
	fn fmt(&self, fmt: &mut Formatter) -> Result {
	    let is_positive: bool = false;
	    let prefix: &'static str = "";
	    let buf: &str = &format!("[{}]", self.value);

	    fmt.pad_integral(is_positive, prefix, buf)
	}
    }

    impl Display for A<i8> {
	fn fmt(&self, fmt: &mut Formatter) -> Result {
    ...
    impl LowerHex for A<u32> {
	fn fmt(&self, fmt: &mut Formatter) -> Result {
	    let is_positive: bool = false;
	    let prefix: &'static str = "Hello, World!";
	    let buf: &str = &format!("[{}]", self.value);
    ...
    #[test]
    fn pad_integral_test1() {
	let a: A<u8> = A { value: 68 };
	let output: String = format!("{:+}", a); // formated with sign

	assert_eq!(output, "-[68]".to_string());
    }

    #[test]
    fn pad_integral_test2() {
	let a: A<i8> = A { value: 68 };
	let output: String = format!("{:+}", a); // format with sign

	assert_eq!(output, "+[68]".to_string());
    }

    #[test]
    fn pad_integral_test3() {
	let a: A<u16> = A { value: 68 };
	let output: String = format!("{:-}", a); // format with sign
    ...
    #[test]
    fn pad_integral_test4() {
	let a: A<i16> = A { value: 68 };
	let output: String = format!("{:-}", a); // format with sign
    ...
    #[test]
    fn pad_integral_test5() {
	let a: A<u32> = A { value: 68 };
	let output: String = format!("{:#x}", a); // format with prefix

	assert_eq!(output, "-Hello, World![68]".to_string());
    }

    #[test]
    fn pad_integral_test6() {
	let a: A<i32> = A { value: 68 };
	let output: String = format!("{:#x}", a); // format with prefix
