...
#[cfg(test)]
mod tests {
    use core::intrinsics::atomic_xor_acq;
    ...
    macro_rules! atomic_xor_acq_test {
	($init:expr, $value:expr, $result:expr) => ({
		...
		let src: T = $value;
		let old: T = unsafe { atomic_xor_acq::<T>(dst, src) };
    ...
    #[test]
    fn atomic_xor_acq_test1() {
	atomic_xor_acq_test!( 0xff00, 0x0a0a, 0xf50a );
    }
}
