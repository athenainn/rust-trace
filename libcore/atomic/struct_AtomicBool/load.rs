...
mod tests {
    ...
    #[test]
    fn load_test1() {
	let atomicbool: AtomicBool = AtomicBool::default();
	...
	let result: bool = data.load(Acquire);
	assert_eq!(result, true);
    }
	...
	let result: bool = data.load(SeqCst);
	assert_eq!(result, true);
    }
    ...
    fn load_test4() {
	let atomicbool: AtomicBool = AtomicBool::default();
	let _: bool = atomicbool.load(Release); // panicked at 'there is no such thing as a release load'
    }
    ...
    fn load_test5() {
	let atomicbool: AtomicBool = AtomicBool::default();
	let _: bool = atomicbool.load(AcqRel); // panicked at 'there is no such thing as an acquire/release load'
    }
}
