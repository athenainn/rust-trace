...
#[cfg(test)]
mod tests {
    use core::str::char_range_at_raw;
    ...
    #[test]
    fn char_range_at_raw_test1() {
	let bytes: &[u8] = &[ 0xe7, 0xb2, 0xbe, 0xe8, 0xaa, 0xa0 ];

	assert_eq!(bytes.len(), 6);

	let (c, end): (u32, usize) = char_range_at_raw(bytes, 0);
	assert_eq!(c, 0x7cbe);
	assert_eq!(end, 3);

	let (c, end): (u32, usize) = char_range_at_raw(bytes, 3);
	assert_eq!(c, 0x8aa0);
	assert_eq!(end, 6);
    }
    ...
    #[test]
    #[should_panic]
    fn char_range_at_raw_test2() {
	let bytes: &[u8] = &[ 0xe9, 0x8a, 0xb9 ];
	...
	let _: (u32, usize) = char_range_at_raw(bytes, 1); // panicked at 'assertion failed: w != 0'
    }

    #[test]
    #[should_panic]
    fn char_range_at_raw_test3() {
	let bytes: &[u8] = &[ 0xe9, 0x8a, 0xb9 ];
	let _: (u32, usize) = char_range_at_raw(bytes, 2); // panicked at 'assertion failed: w != 0'
    }
}
